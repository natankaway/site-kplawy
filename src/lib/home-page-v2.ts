...
const COPY: Record<Locale, Copy> = {
  pt: {
    ...
    demoTitle: 'O lance já passou. O KplaWY ainda pega.',
    demoText: 'Assista ao replay real: o toque no relógio aparece depois da jogada e o clipe nasce dos segundos anteriores.',
    demoQuestion: 'Veja o replay nascer.',
    demoHint: 'Sem correr até o celular. Sem gravar a partida inteira.',
    savedPrefix: 'Últimos',
    savedSuffix: 'salvos.',
  ...
  en: {
    ...
    demoTitle: 'The play is already over. KplaWY still catches it.',
    demoText: 'Watch the real replay: the watch tap happens after the play, and the clip is built from the seconds before it.',
    demoQuestion: 'Watch the replay appear.',
    demoHint: 'No running back to the phone. No recording the whole game.',
    savedPrefix: 'Last',
    savedSuffix: 'saved.',
...
export function renderLandingPage(locale: Locale) {
  ...
  const screens = c.screens.map(([file, title, description, alt], index) => {
    ...
    const screenClass = ['screen-card', index === 0 ? 'featured' : '', index === 3 ? 'result' : '', 'reveal'].filter(Boolean).join(' ');
    return `<article class="${screenClass}"><div class="screen-phone"><img ${load} decoding="async" src="${media}/${file}" width="${width}" height="${height}" alt="${esc(alt)}"></div><div class="screen-copy"><h3>${esc(title)}</h3><p>${esc(description)}</p></div></article>`;
  }).join('');
