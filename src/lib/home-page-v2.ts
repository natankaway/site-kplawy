...
type Copy = {
  ...
  screensTitle: string;
  screensText: string;
  screenNames: [string, string, string, string, string, string];
  screenDescriptions: [string, string, string, string, string, string];
  plansEyebrow: string;
  plansTitle: string;
  plansText: string;
  free: string;
...
const C: Record<Locale, Copy> = {
  pt: {
    ...
    heroBadge: 'REPLAY INSTANTÂNEO NO SEU CELULAR',
    hero1: 'O lance já aconteceu?',
    hero2: 'Salve os últimos segundos.',
    ...
    micro: ['iOS + Android', 'Grava offline', 'Clipes no dispositivo', 'Sem anúncios'],
    demoEyebrow: 'EXPERIMENTE A IDEIA',
    demoTitle: 'Aperte depois. O KPLAWY volta no tempo.',
    demoText: 'O vídeo ao lado foi gerado pelo app. Escolha a duração e simule o momento em que você apertaria REPLAY depois do lance.',
    demoHint: 'O buffer continua rodando; só o trecho escolhido vira um clipe.',
    howEyebrow: 'SIMPLES DE PROPÓSITO',
    howTitle: 'Três passos. Sem gravar horas para aproveitar segundos.',
    howText: 'A lógica do KPLAWY cabe em uma partida.',
    steps: [
    ...
    remoteEyebrow: 'NÃO VOLTE ATÉ A CÂMERA',
    ...
    watchText: 'A tela do relógio agora segue a nova identidade KPLAWY e deixa a ação principal impossível de confundir.',
    screensEyebrow: 'PRODUTO REAL',
    screensTitle: 'Estas são as telas atuais do KPLAWY.',
    screensText: 'A landing agora mostra a interface atual do app, no mesmo idioma do site.',
    screenNames: ['Início', 'Câmera', 'Clipes', 'Player', 'Controle remoto', 'Multi-câmera'],
    screenDescriptions: ['Escolha como usar o KPLAWY.', 'Buffer ativo e REPLAY ao alcance.', 'Clipes por data, filtros e backup.', 'Revise o lance em diferentes velocidades.', 'Conecte outro celular por QR.', 'Vários ângulos do mesmo lance.'],
    plansEyebrow: 'COMECE GRÁTIS',
    plansTitle: 'O essencial é Free. O sistema completo é Pro.',
    plansText: 'Teste o conceito de replay sem pagar e evolua quando precisar de recursos avançados.',
    free: 'Free',
    freePrice: 'R$ 0',
    ...
    privacyText: 'Os clipes permanecem no seu dispositivo, salvo quando você escolhe usar recursos de backup. O site não promete “zero dados”: conta, compra, diagnóstico e melhoria do produto podem exigir dados mínimos.',
    privacyItems: [['Sem rastreamento publicitário', 'O KPLAWY não precisa seguir você entre apps para entregar o replay.'], ['Clipes no dispositivo', 'Os replays locais ficam na sua biblioteca até você decidir compartilhar ou fazer backup.'], ['Backup opcional', 'A nuvem entra apenas quando você escolhe proteger seus arquivos.']],
    ...
    faqs: [
      ['O app grava horas escondido?', 'Não. O buffer mantém apenas uma janela recente para que você salve os últimos segundos quando decidir apertar REPLAY.'],
    ...
    finalText: 'O próximo lance não precisa avisar que vai acontecer.',
  ...
  en: {
    ...
    heroBadge: 'INSTANT REPLAY ON YOUR PHONE',
    hero1: 'The play already happened?',
    hero2: 'Save the last seconds.',
    ...
    micro: ['iOS + Android', 'Records offline', 'Clips on device', 'No ads'],
    demoEyebrow: 'TRY THE IDEA',
    demoTitle: 'Tap afterwards. KPLAWY goes back in time.',
    demoText: 'The video beside this panel was generated by the app. Pick a duration and simulate the moment you would tap REPLAY after the play.',
    demoHint: 'The buffer keeps moving; only the selected window becomes a clip.',
    howEyebrow: 'SIMPLE ON PURPOSE',
    ...
    remoteEyebrow: 'DO NOT WALK BACK TO THE CAMERA',
    ...
    watchText: 'The watch screen now follows the new KPLAWY identity and keeps the main action impossible to miss.',
    screensEyebrow: 'REAL PRODUCT',
    screensTitle: 'These are the current KPLAWY screens.',
    screensText: 'The landing now shows the current app interface in the same language as the site.',
    screenNames: ['Home', 'Camera', 'Clips', 'Player', 'Remote control', 'Multi-camera'],
    screenDescriptions: ['Choose how to use KPLAWY.', 'Active buffer and REPLAY within reach.', 'Clips by date, filters and backup.', 'Review the play at multiple speeds.', 'Connect another phone with QR.', 'Multiple angles of the same play.'],
    ...
    privacyText: 'Clips stay on your device unless you choose to use backup. The site does not make a vague “zero data” claim: account, purchase, diagnostics and product improvement can require minimal data.',
    ...
    faqs: [
      ['Does the app secretly record hours of video?', 'No. The buffer only keeps a recent window so you can save the last seconds when you decide to tap REPLAY.'],
...
const list = (items: string[]) => items.map((x) => `<li>${esc(x)}</li>`).join('');
const faq = (items: [string, string][]) => items.map(([q, a]) => `<details><summary>${esc(q)}</summary><p>${esc(a)}</p></details>`).join('');

export function renderLandingPage(locale: Locale) {
  const c = C[locale];
  const p = `/media/v2/${locale}`;
  const screenFiles = ['home.webp', 'camera.webp', 'clips.webp', 'player.webp', 'remote.webp', 'multicam.webp'];
  const screens = screenFiles.map((file, i) => `<article class="screen-card reveal"><div class="screen-phone"><img loading="lazy" decoding="async" src="${p}/${file}" alt="${esc(c.screenNames[i])} — KPLAWY"></div><h3>${esc(c.screenNames[i])}</h3><p>${esc(c.screenDescriptions[i])}</p></article>`).join('');
  const proof = [['iPhone + Android', locale === 'pt' ? 'um app, qualquer quadra' : 'one app, any court'], ['Apple Watch + Wear OS', locale === 'pt' ? 'dispare sem voltar até a câmera' : 'trigger without walking back'], ['Bluetooth + phone', locale === 'pt' ? 'controle à distância' : 'control at a distance'], ['Multi-camera', locale === 'pt' ? 'mais ângulos do mesmo lance' : 'more angles of the same play']].map(([a, b]) => `<div class="proof-item"><b>${a}</b><span>${b}</span></div>`).join('');
  ...
  const appSchema = { '@context': 'https://schema.org', '@type': 'SoftwareApplication', name: 'KPLAWY', operatingSystem: 'iOS, Android, watchOS, Wear OS', applicationCategory: 'SportsApplication', description: c.description, url: `https://kplawy.app/${locale}`, installUrl: [APP_STORE, PLAY_STORE], offers: locale === 'pt' ? [{ '@type': 'Offer', name: 'KPLAWY Free', price: '0', priceCurrency: 'BRL' }, { '@type': 'Offer', name: 'KPLAWY Pro Monthly', price: '29.90', priceCurrency: 'BRL' }, { '@type': 'Offer', name: 'KPLAWY Pro Annual', price: '249.90', priceCurrency: 'BRL' }] : [{ '@type': 'Offer', name: 'KPLAWY Free', price: '0', priceCurrency: 'USD' }, { '@type': 'Offer', name: 'KPLAWY Pro Monthly', price: '4.99', priceCurrency: 'USD' }, { '@type': 'Offer', name: 'KPLAWY Pro Annual', price: '39.99', priceCurrency: 'USD' }] };
